...
describe("TokenEscrow", function () {
  ...
  let convToken: Contract, tokenEscrow: Contract;
  ...
  beforeEach(async function () {
    ...
    // Contract `NaffitiToken` is simple enough to not use a mock
    convToken = await NaffitiToken.deploy(
      owner.address // genesis_holder
    );

    tokenEscrow = await TokenEscrow.deploy();
    await tokenEscrow.connect(deployer).__TokenEscrow_init(
      convToken.address // _token
    );
    ...
    await convToken.connect(owner).transfer(
